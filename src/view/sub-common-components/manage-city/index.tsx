        ...
        {cityList.length > 0 ? (
          Object.keys(cityList).map((cityId: any) => (
            <div className={styles.cityItem} key={cityId}>
              ...
              <Button title="Submit" onClick={() => OnGetCityInformation(cityList[cityId])} />
              <Button title="Remove" onClick={() => OnRemoveCityFromList(cityList[cityId])} />
            </div>
          ))
        ) : (
          <div>City list is empty.</div>
        )}
